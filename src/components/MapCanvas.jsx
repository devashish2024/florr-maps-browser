...
export default function MapCanvas({ mapData, sprites, mobSprites, mapKey, onMapChange, cameraTarget, onCameraTargetApplied }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dpr = cfg.halfCanvasResolution ? Math.max(1, devicePixelRatio * 0.5) : devicePixelRatio;
      st.dpr = dpr;
      ...
      overlay.width = w;
      overlay.height = h;
      // Tooltips canvas always at same resolution as main
      uiCanvas.width = w;
      uiCanvas.height = h;
      st.uiScale = 1;
    ...
    if (firstMapRef.current) {
      ...
      if (cameraTarget) {
        // Glide from map center to the warp/warp_destination position
        // x/y = target the camera lerps toward; rx/ry = where rendering starts
